...
class PlaylistDownloadService {
  ...
  async downloadAndStoreSong(song: Song, baseUrl: string): Promise<void> {
    try {
      ...
      if (!response.ok) throw new Error(`Failed to download song: ${song.name}`);
  ...
  private async storeSong(songId: string, blob: Blob): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        console.log(`Song stored in IndexedDB: ${songId}`);
        resolve();
      };
      request.onerror = () => {
        console.error(`Error storing song: ${songId}`, request.error);
  ...
  async getSong(songId: string): Promise<Blob | null> {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        console.error(`Error retrieving song: ${songId}`, request.error);
        reject(request.error);
      };
    });
  }
